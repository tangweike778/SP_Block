    ...
    public static void findSubTensor(List<Integer> globalList, PriorityQueue<Pair<Tensor, Double>> subTensorPriorityQueue, int k, int circleNum) throws CloneNotSupportedException {
        while(true){
            ...
            for(int mode = 0; mode < 3; mode++){
                int len = stMass[mode].length;
                for(int i = 0; i < len; i++){
                    if(stMass[mode][i] != 0 && stMass[mode][i] < minValue){
                        dimension = mode;
                        attr = i;
                        minValue = stMass[mode][i];
                    }
                }
            }
            ...
            for (Integer j : globalList) {
                int tempAttr = attVals[Converge.TIME_COLUMN + dimension][j];
                if (tempAttr > attr) {
                    dataList2.add(j);
                } else {
                    dataList1.add(j);
            ...
            //RPA_PII
            if(Util.preDeleteSlice(attr, Converge.TIME_COLUMN + dimension, globalList) > measurementBefore){
                long startTime = System.currentTimeMillis();
                Util.deleteSlice(attr, Converge.TIME_COLUMN + dimension, globalList);
