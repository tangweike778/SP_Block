...
public class SubTensor implements Serializable {
    ...
    public SubTensor(int mass, int minTime, int maxTime, double minLongitude, double maxLongitude, double minLatitude, double maxLatitude, int[] size, boolean[] timeExisted, List<boolean[]> coreDimensionExisted,boolean[] latitudeExisted, boolean[] longitudeExisted, List<int[]> coreDimensionMass, int[] timeMass, int[] timeSpaceMass, int[] latitudeMass, int[] longitudeMass, int[][] attVals, double[][] spaceVals, int[] value, List<Map<Integer, List<Integer>>> selectDataByCoreDimension, Map<Integer, List<Integer>> selectDataByLatitude, Map<Integer, List<Integer>> selectDataByLongitude) {
        this.mass = mass;
    ...
    public double imitateAppendDatas(Tensor tensor, List<Integer> dataList) {
        ...
        List<Set<Double>> stSize = new ArrayList<>();
        for (int i = 0; i < 3; i++) {
            stSize.add(new HashSet<>());
        }
        for (Integer key : selectDataByLatitude.keySet()) {
            List<Integer> valueSet = selectDataByLatitude.get(key);
            for (Integer index : valueSet) {
                if(value[index] == 0)continue;
                stSize.get(0).add((double) attVals[Converge.TIME_COLUMN][index]);
                stSize.get(1).add(spaceVals[0][index]);
                stSize.get(2).add(spaceVals[1][index]);
            }
        }
        ...
        for(Integer data : dataList){
            ...
            stSize.get(0).add((double) attVals[Converge.TIME_COLUMN][data]);
            stSize.get(1).add(spaceVals[0][data]);
            stSize.get(2).add(spaceVals[1][data]);
        ...
        for (Set<Double> set : stSize) {
            totalSize += set.size();
        }

        // return Util.computeDensity(mass, coreSize);
        return Util.computeMeasurement(Util.computeDensity(mass, totalSize),spatialRange);
    }
