...
public class Util {
    ...
    public static double computeDensityBaseCardinality(SubTensor subTensor){
        int allSize = 0;
        ...
        for (int i = 0; i < Converge.globalSize.length - 1; i++) {
            if(i < Converge.TIME_COLUMN){
                ...
                allSize += stSize.get(i - Converge.TIME_COLUMN).size();
            }
        }
        return subTensor.getMass() * (Converge.globalSize.length - 1) * 1.0 / allSize;
    }
    ...
    public static int computeTimeSpaceIndex(int time, double latitude, double longitude){
        return Converge.PARTITION_NUMBER * Converge.PARTITION_NUMBER + computeSpaceIndex(latitude, longitude);
    }
    ...
    public static double computeTotalDensity(SubTensor subTensor){
        ...
        //TODO：修改了度量
        double density = subTensor.getMass() * (float)(Converge.globalSize.length - 1) / computeVolumeBaseCardinality(subTensor);
        ...
        return computeMeasurement(density, spatialRange);
    }
...
/*    public static double computeWeightDensity(SubTensor subTensor){
     ...
     * @return 子张量体积
     */
    public static long computeVolumeBaseCardinality(SubTensor subTensor){
        long allSize = 0;
        int[] size = subTensor.getSize();
        int[] value = subTensor.getValue();
        List<Set<Double>> stSize = new ArrayList<>();
