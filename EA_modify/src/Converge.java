...
public class Converge{
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Coreprocess处理核心维度的时间为：" + demoTime1 + "ms");
        System.out.println("扩展操作的时间为" + (demoTime2 - demoTime1) + "ms");
        System.out.println("demoTime3 " + demoTime3 + "ms");
    ...
    private static void preComputeParameter(String input_path, String splitStr) throws Exception {
        ...
        // timeStep = (globalMaxTime - globalMinTime + 1) * 1.0 / PARTITION_NUMBER;
        latitudeStep = (globalMaxLatitude - globalMinLatitude + 1) / PARTITION_NUMBER;
        longitudeStep = (globalMaxlongitude - globalMinlongitude + 1) / PARTITION_NUMBER;
    }
...
/*    private static void computeTemporalSpatialStep(){
    ...
    public static void outputTensorList(Tensor tensor, List<SubTensor> tensorList, String output_path, int totalNum) {
        ...
        try {
            ...
            for (int i = 0; i < totalNum; i++) {
                ...
                System.out.println("Valume: " + Util.computeVolumeBaseCardinality(subTensor));
                System.out.println("Density: " + Util.computeDensityBaseCardinality(subTensor));
    ...
    public static void outputProperties(Tensor tensor, SubTensor subTensor, int i, String output_path) throws IOException {
        ...
        sb1.append("Valume: "+ Util.computeVolumeBaseCardinality(subTensor) +"\n");
        sb1.append("Mass: "+ Util.computeDensity(subTensor.getMass(), Util.computeVolumeBaseCardinality(subTensor)) +"\n");
    ...
    public static List<SubTensor> findSubTensors(Tensor tensor, int k) {
        PriorityQueue<Pair<SubTensor, Double>> subTensorPriorityQueue = new PriorityQueue<>(k, new Comparator<Pair<SubTensor, Double>>() {
            ...
            public int compare(Pair<SubTensor, Double> o1, Pair<SubTensor, Double> o2) {
                double a = Util.computeDensityBaseCardinality(o1.getKey());
                double b = Util.computeDensityBaseCardinality(o2.getKey());
        ...
        demoTime2 += System.currentTimeMillis() - start2;
        System.gc();
    ...
    public static void findSubTensorsInOneRound(Tensor tensor, PriorityQueue<Pair<SubTensor, Double>> subTensorPriorityQueue, int k, int circleNum) {
        ...
        for (int i = 0; i < timeSpaceIndexsBySortMass.length; i++) {
            ...
            Util.setSigmoid_base(Util.computeDensityBaseCardinality(subTensor));
            while(true){
                long startTime = System.currentTimeMillis();
                ...
                if(densityBatchAdd > maxDensity){
                    ...
                    demoTime1 += System.currentTimeMillis() - startTime;
                    break;
                }
                // System.out.println("扩展次数为："+EXPAND_NUMBER);
            }
    ...
    public static SubTensor createSubTensorByDatas(Tensor tensor, List<Integer> dataList) {
        ...
        for(int i = 0; i < CORE_COLUMN_LENGTH; i++){
            if(i < TIME_COLUMN){
                ...
                newCoreDimensionExisted.add(new boolean[globalMaxTime + 1]);
            }
        }
        ...
        for(int i = 0; i < CORE_COLUMN_LENGTH; i++){
            if(i < TIME_COLUMN){
                ...
                newCoreDimensionMass.add(new int[globalMaxTime + 1]);
            }
        }
        ...
        for(Integer data : dataList){
            ...
            //计算时空块索引地址
            int timeSpaceBlockIndex = Util.computeDirectSpaceIndex(processedLatitude, processedLongitude);
            //处理经纬度质量是向上取整
        ...
        return new SubTensor(newMass, newMinTime, newMaxTime, newMinLongitude, newMaxLongitude, newMinLatitude, newMaxLatitude, newSize, newTimeExisted, newCoreDimensionExisted, newLatitudeExisted, newLongitudeExisted, newCoreDimensionMass, newTimeMass,  newTimeSpaceMass, newLatitudeMass, newLongitudeMass, newAttVals, newSpaceVals, newValue, newSelectDataByCoreDimension, newSelectDataByLatitude, newSelectDataByLongitude);
    }
    ...
    public static Tensor createTensor(String file_path, String splitStr){
        ...
        for(int i = 0; i < CORE_COLUMN_LENGTH; i++){
            if(i < TIME_COLUMN){
                ...
                coreDimensionMass.add(new int[globalMaxTime + 1]);
            }
        }
        int timeSpaceMaxIndex = PARTITION_NUMBER * PARTITION_NUMBER + PARTITION_NUMBER * PARTITION_NUMBER;
        ...
        //modeId用于给属性值进行转换
        //初始化modeId,全部填充为1
        Integer[] modeId = new Integer[CORE_COLUMN_LENGTH];
        ...
        try {
            ...
            while ((line = reader.readLine()) != null){
                ...
                //计算时空块索引地址
                int timeSpaceIndex = Util.computeSpaceIndex(latitude, longitude);
        ...
        return new Tensor(mass, size, attVals, spaceVals, value, coreDimensionMass,  timeSpaceMass , latitudeMass, longitudeMass, timeSpaceExisted ,newSelectDataByCoreDimension,  selectDataByTimeSpace);
    }
